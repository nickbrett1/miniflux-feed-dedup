...
# "miniflux-feed-dedup-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "miniflux-feed-dedup-dev" is overwritten by
...
PROJECT_NAME="miniflux-feed-dedup"
AGENT_NAME="miniflux-feed-dedup-dev"
WORKSPACE="/workspaces/miniflux-feed-dedup"
...
if [ -x "/workspaces/miniflux-feed-dedup/scripts/agent-dev.sh" ]; then
  "/workspaces/miniflux-feed-dedup/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
