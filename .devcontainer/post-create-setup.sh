...
if [ -f "/workspaces/miniflux-feed-dedup/.devcontainer/.zshrc" ]; then
    echo "INFO: Copying .zshrc to $USER_HOME_DIR/.zshrc"
    cp "/workspaces/miniflux-feed-dedup/.devcontainer/.zshrc" "$USER_HOME_DIR/.zshrc"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.zshrc"
else
    echo "INFO: /workspaces/miniflux-feed-dedup/.devcontainer/.zshrc not found, skipping copy."
fi

if [ -f "/workspaces/miniflux-feed-dedup/.devcontainer/.p10k.zsh" ]; then
    echo "INFO: Copying .p10k.zsh to $USER_HOME_DIR/.p10k.zsh"
    cp "/workspaces/miniflux-feed-dedup/.devcontainer/.p10k.zsh" "$USER_HOME_DIR/.p10k.zsh"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.p10k.zsh"
else
    echo "INFO: /workspaces/miniflux-feed-dedup/.devcontainer/.p10k.zsh not found, skipping copy."
fi

if [ -f "/workspaces/miniflux-feed-dedup/.devcontainer/.tmux.conf" ]; then
    echo "INFO: Copying .tmux.conf to $USER_HOME_DIR/.tmux.conf"
    cp "/workspaces/miniflux-feed-dedup/.devcontainer/.tmux.conf" "$USER_HOME_DIR/.tmux.conf"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.tmux.conf"
else
    echo "INFO: /workspaces/miniflux-feed-dedup/.devcontainer/.tmux.conf not found, skipping copy."
fi
...
cd "/workspaces/miniflux-feed-dedup" 2>/dev/null || true
...
if [ -d "/workspaces/miniflux-feed-dedup/.venv/bin" ]; then
    export PATH="/workspaces/miniflux-feed-dedup/.venv/bin:$PATH"
fi
EOF
...
git config --global --add safe.directory /workspaces/miniflux-feed-dedup
...
echo "    cd /workspaces/miniflux-feed-dedup && bash scripts/cloud_login.sh"
